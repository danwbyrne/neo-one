...
const getCombinedModels = (
  ...
  const init = consensusData ? [consensusData] : [];
...
export class Block extends BlockBase implements SerializableWire<Block>, SerializableJSON<BlockJSON> {
  ...
  protected readonly sizeExclusive = utils.lazy(() => {
    const init = this.consensusData ? [this.consensusData] : [];
    return IOHelper.sizeOfArray(init.concat(this.transactions), (transaction) => transaction.size);
  });
